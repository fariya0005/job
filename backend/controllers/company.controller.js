...
export const registerCompany = async (req, res) => {
    try {
        ...
        if (!companyName || !email) { // Only companyName and email are mandatory
            return res.status(400).json({
        ...
        // Create the new company with optional fields
        company = await Company.create({
            name: companyName,
            email,
            phone,           // Optional
            ...
            industry,        // Optional
            description,     // Optional
            password: hashedPassword, // Hash password only if provided
            userId: req.id  // Ensure req.id is correctly passed from the session or token
        });
...
export const getCompany = async (req, res) => {
    try {
        ...
        if (!companies || companies.length === 0) { // Check if no companies were found
